import{A as d}from"./api-BYJfvy6u.js";class f{static async init(){await this.renderPartners(),await this.renderNews(),await this.renderServices(),await this.renderDirectors(),await this.renderGeneral(),this.setupNavigation(),this.setupScrollHeader(),this.setupJoinForm()}static setupScrollHeader(){const a=document.getElementById("navbar");window.addEventListener("scroll",()=>{window.scrollY>50?(a.classList.add("bg-secondary-dark/95","shadow-lg"),a.classList.remove("bg-gradient-to-b","from-secondary-dark/90","to-transparent")):(a.classList.remove("bg-secondary-dark/95","shadow-lg"),a.classList.add("bg-gradient-to-b","from-secondary-dark/90","to-transparent"))})}static async renderDirectors(){const a=await d.getDirectors(),o=document.getElementById("directors-grid");if(o&&a.length>0){o.innerHTML="";const n={1:{name:"Antonio Henrique Sapori",role:"Presidente",company:"RODONASA CARGAS E ENCOMENDAS LTDA."},2:{name:"Lucio Bento Fagundes Junior",role:"Secretário",company:"RÁPIDO MONTES CLAROS E FERNANDO SOARES MOTA - ME."},3:{name:"Paulo Roberto de Almeida",role:"Tesoureiro",company:"TRANSPORTADORA JANUÁRIA"},4:{name:"Antonio Sapori",role:"Conselho Fiscal Efetivo",company:"TRANSNORTE CARGAS E ENCOMENDAS LTDA."},5:{name:"Jorge Antonio dos Santos",role:"Conselho Fiscal Efetivo",company:"JLSI Logística e Transportes Ltda.-ME"},6:{name:"Antonio Henrique Sapori Filho",role:"Conselho Fiscal Suplente",company:"PRAMINAS LOG TRANSPORTES LTDA."}},e=a.map(t=>{const r=n[t.id];return r?{...t,...r}:t});e.sort((t,r)=>parseInt(t.id)-parseInt(r.id)),e.forEach(t=>{const c=!t.image||t.image.includes("placeholder")?'<div class="w-16 h-16 rounded-full bg-gray-700 flex items-center justify-center text-gray-400 text-3xl border-2 border-transparent group-hover:border-accent-cyan transition-colors"><i class="fas fa-user"></i></div>':`<img src="${t.image}" alt="${t.name}" class="w-16 h-16 rounded-full object-cover border-2 border-transparent group-hover:border-accent-cyan transition-colors">`;o.innerHTML+=`
                    <div class="bg-secondary-dark border border-glass-border p-6 rounded-xl flex items-center gap-4 hover:border-accent-cyan transition-colors group">
                        ...
                        <div>
                            <h4 class="font-heading font-bold text-white text-lg leading-tight mb-1">${t.name}</h4>
                            <p class="text-accent-cyan text-xs font-bold uppercase tracking-widest mb-1">${t.role}</p>
                            <p class="text-gray-500 text-xs">${t.company}</p>
                        </div>
                    </div>
                `})}}static async renderPartners(){const a=await d.getPartners(),o=document.getElementById("partners-grid");if(o&&a.length>0){const t=`<div class="flex gap-16 shrink-0 items-center px-8">${a.map(r=>`
                <a href="parceiro.html?id=${r.id}" class="flex items-center justify-center min-w-[200px] grayscale opacity-40 hover:grayscale-0 hover:opacity-100 transition-all duration-500 cursor-pointer group">
            ...
            `).join("")}</div>`;o.innerHTML=new Array(12).fill(t).join(""),o.className="flex items-center w-max animate-marquee md:animate-marquee-slow"}const n=document.querySelector("footer img");n&&(n.onerror=()=>{n.src="assets/favicon.png"},n.src="assets/favicon.png")}static async renderNews(){const a=await d.getNews(),o=document.getElementById("news-grid");o&&a.length>0&&(o.innerHTML="",a.slice(0,3).forEach(e=>{o.innerHTML+=`
                    <a href="/noticia?id=${e.id}" class="block bg-primary-dark rounded-lg overflow-hidden group hover:-translate-y-2 transition-all duration-300 shadow-[0_10px_30px_rgba(0,0,0,0.5)] border border-glass-border">
                        ...
                        <div class="p-8">
                            <div class="flex items-center gap-3 mb-4">
                                ...
                                <span class="text-gray-400 text-xs font-bold uppercase tracking-widest">${e.news_date||e.date||"Recent"}</span>
                            </div>
                ...
                `}))}static async renderServices(){const a=await d.getServices(),o=document.getElementById("services-grid");if(o&&a.length>0){o.innerHTML="";const n={"Registro RNTRC (ANTT)":{description:"O Sindinor é credenciado junto à Agência Nacional de Transportes Terrestres (ANTT) para realização do Registro e Recadastramento Nacional de Transportador Rodoviário de Cargas (RNTRC).",contact_info:{title:"Mais informações:",phone:"(38) 3321-9110 / (38) 99985-3216",email:"ercadastro@gmail.com.br",contact:"Edson Ricardo"},logos:["/assets/antt-logo.jpg","/assets/rntrc-logo.jpg"]},"Programa Despoluir":{description:"O Despoluir é um Programa Ambiental de Transporte que objetiva a redução de emissões de poluentes. Gratuitamente, as empresas associadas Sindinor podem aferir sua frota.",extra_description:'Os veículos aprovados recebem o <strong class="text-white">Selo Verde Despoluir</strong> e empresas de destaque concorrem ao Prêmio Melhor Ar, promovido pelo Fetcemg.'},"Consultoria Técnica":{description:"O Sindinor oferece assessoria econômica para auxiliar na elaboração de planilhas de custos, formação de preços e fornecimento de indicadores econômicos atualizados.",extra_description:"Seguindo as orientações do Índice Nacional de Custos de Transporte de Carga Fracionada e Lotação, garantindo competitividade e sustentabilidade ao negócio."},"Consultoria Técnica e Econômica":{description:"O Sindinor oferece assessoria econômica para auxiliar na elaboração de planilhas de custos, formação de preços e fornecimento de indicadores econômicos atualizados.",extra_description:"Seguindo as orientações do Índice Nacional de Custos de Transporte de Carga Fracionada e Lotação, garantindo competitividade e sustentabilidade ao negócio."}};a.forEach(e=>{const t=n[e.title],r=t&&e.description.length<50?t.description:e.description,c=e.extra_description||(t?t.extra_description:""),i=e.contact_info||(t?t.contact_info:null),l=e.logos&&e.logos.length>0?e.logos:t?t.logos:[];let s="";l&&l.length>0?s=`
                    ...
                    `);const p=c?`<p class="text-gray-400 text-sm leading-relaxed mb-6">${c}</p>`:"",u=e.icon?`<i class="fas ${e.icon} text-9xl"></i>`:`<img src="${e.image}" class="w-24 h-24 filter invert opacity-50">`;o.innerHTML+=`
                    <a href="servico.html?id=${e.id}" class="bg-secondary-dark rounded-lg p-8 hover:-translate-y-2 transition-transform duration-300 relative overflow-hidden group shadow-lg border border-white/5 flex flex-col h-full block">
                        ...
                        <div class="relative z-10 flex flex-col h-full">
                            ...
                            <p class="text-gray-400 text-sm leading-relaxed mb-4">${r}</p>
                            ${p}
                            ${g}
                ...
                `})}}static async renderGeneral(){const a=await d.getGeneral();a&&a.hero}static setupNavigation(){const a=document.querySelectorAll("section"),o=document.querySelectorAll("nav a"),n=document.querySelectorAll("#mobile-menu a"),e=new IntersectionObserver(t=>{t.forEach(r=>{if(r.isIntersecting){const c=r.target.getAttribute("id");o.forEach(s=>{s.classList.remove("text-accent-cyan"),s.classList.add("text-white/90"),s.getAttribute("href")==="#associe-se"&&(s.classList.remove("text-accent-cyan"),s.classList.add("text-accent-cyan"))}),n.forEach(s=>{s.classList.remove("text-accent-cyan"),s.classList.add("text-white"),s.getAttribute("href")==="#associe-se"&&s.classList.add("text-accent-cyan")});const i=document.querySelector(`nav a[href="#${c}"]`);i&&(i.classList.remove("text-white/90"),i.classList.add("text-accent-cyan"));const l=document.querySelector(`#mobile-menu a[href="#${c}"]`);l&&(l.classList.remove("text-white"),l.classList.add("text-accent-cyan"))}})},{threshold:.2,rootMargin:"-10% 0px -50% 0px"});a.forEach(t=>e.observe(t))}static setupJoinForm(){const a=document.getElementById("join-form");a&&a.addEventListener("submit",async o=>{o.preventDefault();const n=a.querySelector('button[type="submit"]'),e=n.innerText,t={company:document.getElementById("join-company").value,cnpj:document.getElementById("join-cnpj").value,city:document.getElementById("join-city").value,name:document.getElementById("join-name").value,role:document.getElementById("join-role").value,phone:document.getElementById("join-phone").value,message:document.getElementById("join-message").value};try{n.innerText="Enviando...",n.disabled=!0,await d.addRequest(t),alert("Solicitação enviada com sucesso! Em breve entraremos em contato."),a.reset(),window.toggleJoinModal&&window.toggleJoinModal()}catch(r){console.error("Error submitting request:",r),alert("Erro ao enviar solicitação. Tente novamente.")}finally{n.innerText=e,n.disabled=!1}})}}document.addEventListener("DOMContentLoaded",()=>{f.init()});
