import{A as i}from"./api-B-8y-DDH.js";class g{static async init(){await this.renderPartners(),await this.renderNews(),await this.renderServices(),await this.renderDirectors(),await this.renderGeneral(),this.setupNavigation(),this.setupScrollHeader(),this.setupJoinForm()}static setupScrollHeader(){const t=document.getElementById("navbar");window.addEventListener("scroll",()=>{window.scrollY>50?(t.classList.add("bg-secondary-dark/95","shadow-lg"),t.classList.remove("bg-gradient-to-b","from-secondary-dark/90","to-transparent")):(t.classList.remove("bg-secondary-dark/95","shadow-lg"),t.classList.add("bg-gradient-to-b","from-secondary-dark/90","to-transparent"))})}static async renderDirectors(){const t=await i.getDirectors(),a=document.getElementById("directors-grid");a&&t.length>0&&(a.innerHTML="",t.forEach(e=>{const s=!e.image||e.image.includes("placeholder")?'<div class="w-16 h-16 rounded-full bg-gray-700 flex items-center justify-center text-gray-400 text-3xl border-2 border-transparent group-hover:border-accent-cyan transition-colors"><i class="fas fa-user"></i></div>':`<img src="${e.image}" alt="${e.name}" class="w-16 h-16 rounded-full object-cover border-2 border-transparent group-hover:border-accent-cyan transition-colors">`;a.innerHTML+=`
            ...
            `).join("");a.innerHTML=e+e+e+e,a.classList.add("animate-marquee")}}static async renderNews(){const t=await i.getNews(),a=document.getElementById("news-grid");a&&t.length>0&&(a.innerHTML="",t.slice(0,3).forEach(r=>{a.innerHTML+=`
                ...
                `}))}static async renderServices(){const t=await i.getServices(),a=document.getElementById("services-grid");a&&t.length>0&&(a.innerHTML="",t.forEach(e=>{const r=e.icon?`<i class="fas ${e.icon} text-4xl text-accent-cyan group-hover:scale-110 transition-transform"></i>`:`<img src="${e.image}" alt="${e.title}" class="w-8 h-8 filter invert group-hover:invert-0 transition-all">`,s=e.icon?`<i class="fas ${e.icon} text-9xl"></i>`:`<img src="${e.image}" class="w-24 h-24 filter invert opacity-50">`;a.innerHTML+=`
                ...
                `}))}static async renderGeneral(){const t=await i.getGeneral();t&&t.hero}static setupNavigation(){const t=document.querySelectorAll("section"),a=document.querySelectorAll("nav a"),e=document.querySelectorAll("#mobile-menu a"),r=new IntersectionObserver(s=>{s.forEach(o=>{if(o.isIntersecting){const d=o.target.getAttribute("id");a.forEach(n=>{n.classList.remove("text-accent-cyan"),n.classList.add("text-white/90"),n.getAttribute("href")==="#associe-se"&&(n.classList.remove("text-accent-cyan"),n.classList.add("text-accent-cyan"))}),e.forEach(n=>{n.classList.remove("text-accent-cyan"),n.classList.add("text-white"),n.getAttribute("href")==="#associe-se"&&n.classList.add("text-accent-cyan")});const c=document.querySelector(`nav a[href="#${d}"]`);c&&(c.classList.remove("text-white/90"),c.classList.add("text-accent-cyan"));const l=document.querySelector(`#mobile-menu a[href="#${d}"]`);l&&(l.classList.remove("text-white"),l.classList.add("text-accent-cyan"))}})},{threshold:.2,rootMargin:"-10% 0px -50% 0px"});t.forEach(s=>r.observe(s))}static setupJoinForm(){const t=document.getElementById("join-form");t&&t.addEventListener("submit",async a=>{a.preventDefault();const e=t.querySelector('button[type="submit"]'),r=e.innerText,s={company:document.getElementById("join-company").value,cnpj:document.getElementById("join-cnpj").value,city:document.getElementById("join-city").value,name:document.getElementById("join-name").value,role:document.getElementById("join-role").value,phone:document.getElementById("join-phone").value};try{e.innerText="Enviando...",e.disabled=!0,await i.addRequest(s),alert("Solicitação enviada com sucesso! Em breve entraremos em contato."),t.reset(),window.toggleJoinModal&&window.toggleJoinModal()}catch(o){console.error("Error submitting request:",o),alert("Erro ao enviar solicitação. Tente novamente.")}finally{e.innerText=r,e.disabled=!1}})}}document.addEventListener("DOMContentLoaded",()=>{g.init()});
