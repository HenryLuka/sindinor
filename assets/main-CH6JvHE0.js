import{A as i}from"./api-q9fuA95L.js";class g{static async init(){await this.renderPartners(),await this.renderNews(),await this.renderServices(),await this.renderDirectors(),await this.renderGeneral(),this.setupNavigation(),this.setupScrollHeader(),this.setupJoinForm()}static setupScrollHeader(){const t=document.getElementById("navbar");window.addEventListener("scroll",()=>{window.scrollY>50?(t.classList.add("bg-secondary-dark/95","shadow-lg"),t.classList.remove("bg-gradient-to-b","from-secondary-dark/90","to-transparent")):(t.classList.remove("bg-secondary-dark/95","shadow-lg"),t.classList.add("bg-gradient-to-b","from-secondary-dark/90","to-transparent"))})}static async renderDirectors(){const t=await i.getDirectors(),a=document.getElementById("directors-grid");a&&t.length>0&&(a.innerHTML="",t.forEach(e=>{const n=!e.image||e.image.includes("placeholder")?'<div class="w-16 h-16 rounded-full bg-gray-700 flex items-center justify-center text-gray-400 text-3xl border-2 border-transparent group-hover:border-accent-cyan transition-colors"><i class="fas fa-user"></i></div>':`<img src="${e.image}" alt="${e.name}" class="w-16 h-16 rounded-full object-cover border-2 border-transparent group-hover:border-accent-cyan transition-colors">`;a.innerHTML+=`
                ...
                `}))}static async renderPartners(){const t=await i.getPartners(),a=document.getElementById("partners-grid");if(a&&t.length>0){const r=`<div class="flex gap-16 shrink-0 items-center px-8">${t.map(n=>`
                <div class="flex items-center justify-center min-w-[200px] grayscale opacity-40 hover:grayscale-0 hover:opacity-100 transition-all duration-500 cursor-pointer group">
                     ${n.logo?`<img src="${n.logo}" alt="${n.name}" class="h-24 w-auto object-contain">`:`<span class="text-4xl font-black text-gray-500 group-hover:text-white uppercase tracking-tighter transition-colors select-none">${n.name}</span>`}
                </div>
            `).join("")}</div>`;a.innerHTML=r+r+r+r,a.className="flex items-center animate-marquee"}}static async renderNews(){const t=await i.getNews(),a=document.getElementById("news-grid");a&&t.length>0&&(a.innerHTML="",t.slice(0,3).forEach(r=>{a.innerHTML+=`
