var p=Object.defineProperty;var y=(g,e,a)=>e in g?p(g,e,{enumerable:!0,configurable:!0,writable:!0,value:a}):g[e]=a;var v=(g,e,a)=>y(g,typeof e!="symbol"?e+"":e,a);import{A as d}from"./api-CKE281IM.js";class h{static readFileAsBase64(e){return new Promise((a,i)=>{const s=new FileReader;s.readAsDataURL(e),s.onload=t=>{const n=new Image;n.src=t.target.result,n.onload=()=>{let l=n.width,o=n.height;l>o?l>800&&(o*=800/l,l=800):o>800&&(l*=800/o,o=800);const r=document.createElement("canvas");r.width=l,r.height=o,r.getContext("2d").drawImage(n,0,0,l,o),a(r.toDataURL("image/jpeg",.7))},n.onerror=c=>i(c)},s.onerror=t=>i(t)})}static init(){var e,a,i,s;window.checkAuth=()=>this.checkAuth(),window.logout=()=>this.logout(),localStorage.getItem("sindinor_admin_logged")==="true"&&this.showAdminPanel(),window.switchTab=t=>this.switchTab(t),window.deleteNews=t=>this.deleteNews(t),window.deletePartner=t=>this.deletePartner(t),window.deleteService=t=>this.deleteService(t),window.deleteDirector=t=>this.deleteDirector(t),window.editService=t=>this.editService(t),window.removeServiceGalleryItem=t=>this.removeServiceGalleryItem(t),window.removeNewsGalleryItem=t=>this.removeNewsGalleryItem(t),window.handleDeleteRequest=t=>this.handleDeleteRequest(t),(e=document.getElementById("news-form"))==null||e.addEventListener("submit",t=>this.handleAddNews(t)),(a=document.getElementById("partner-form"))==null||a.addEventListener("submit",t=>this.handleAddPartner(t)),(i=document.getElementById("service-form"))==null||i.addEventListener("submit",t=>this.handleAddService(t)),(s=document.getElementById("director-form"))==null||s.addEventListener("submit",t=>this.handleAddDirector(t)),window.handleDeleteRequest=t=>this.handleDeleteRequest(t)}static checkAuth(){const e=document.getElementById("admin-email").value,a=document.getElementById("admin-pass").value;e==="administrativo@sindinor.org.br"&&a==="Sindi#2025"?(localStorage.setItem("sindinor_admin_logged","true"),this.showAdminPanel()):alert("E-mail ou senha incorretos!")}static showAdminPanel(){document.getElementById("auth-overlay").style.display="none",document.getElementById("admin-content").style.display="flex",this.loadData(),this.switchTab("news")}static logout(){localStorage.removeItem("sindinor_admin_logged"),location.reload()}static switchTab(e){document.querySelectorAll(".tab-content").forEach(i=>{i.classList.add("hidden"),i.classList.remove("active")}),document.querySelectorAll(".nav-btn").forEach(i=>i.classList.remove("active")),document.getElementById(`tab-${e}`).classList.remove("hidden"),document.getElementById(`tab-${e}`).classList.add("active");const a=document.querySelector(`button[onclick="switchTab('${e}')"]`);a&&a.classList.add("active")}static async loadData(){const[e,a,i,s,t]=await Promise.all([d.getNews(),d.getPartners(),d.getServices(),d.getDirectors(),d.getRequests()]),n=document.getElementById("news-list");n&&(n.innerHTML="",e.forEach(r=>{n.innerHTML+=`
                ...
                `}));const l=document.getElementById("directors-list");l&&(l.innerHTML="",s.forEach(r=>{l.innerHTML+=`
                    <div class="bg-secondary-dark border border-glass-border p-4 rounded-lg flex gap-4 items-center group hover:border-accent-cyan transition-colors">
        ...
        `).join(""),this.editingNewsGallery.length===0&&(e.innerHTML='<p class="text-text-muted text-[10px] uppercase tracking-widest italic">Nenhuma foto na galeria</p>'))}static removeNewsGalleryItem(e){this.editingNewsGallery.splice(e,1),this.renderNewsGalleryPreview()}static async handleAddPartner(e){e.preventDefault(),console.log("Adding partner...");const a=document.getElementById("partner-file");let s=document.getElementById("partner-logo").value;if(a.files&&a.files[0]){console.log("Reading file:",a.files[0].name);try{s=await this.readFileAsBase64(a.files[0]),console.log("File read success, base64 length:",s.length)}catch(n){console.error("Error reading file",n),alert("Erro ao ler a imagem! Tente outro arquivo.");return}}if(!s&&!document.getElementById("partner-name").value){alert("Preencha pelo menos o nome!");return}const t={name:document.getElementById("partner-name").value,description:document.getElementById("partner-desc").value,video_url:document.getElementById("partner-video").value,logo:s};try{await d.addPartner(t),console.log("Partner added to storage"),alert("Parceiro Adicionado com Sucesso!"),this.loadData(),e.target.reset()}catch(n){console.error(n),alert("Erro ao salvar: "+n.message)}}static async handleAddService(e){e.preventDefault();const a=document.getElementById("editing-service-id").value,i=document.getElementById("service-file");let t=document.getElementById("service-img").value;if(i.files&&i.files[0])try{t=await this.readFileAsBase64(i.files[0])}catch(r){console.error("Error reading file",r);return}let n=[...this.editingGallery];const c=document.getElementById("service-gallery-urls");if(c.value){const r=c.value.split(",").map(m=>m.trim()).filter(m=>m);n=[...n,...r]}const u=document.getElementById("service-gallery-files");if(u.files&&u.files.length>0)for(let r=0;r<u.files.length;r++)try{const m=await this.readFileAsBase64(u.files[r]);n.push(m)}catch(m){console.error("Error reading gallery file",m)}const l={title:document.getElementById("service-title").value,description:document.getElementById("service-desc").value,full_description:document.getElementById("service-full-desc").value,image:t,gallery:n};a?(await d.updateService(a,l),alert("Serviço Atualizado!")):(await d.addService(l),alert("Serviço Adicionado!")),this.loadData(),e.target.reset(),this.editingGallery=[],this.renderGalleryPreview(),document.getElementById("editing-service-id").value="";const o=document.querySelector("#tab-services h3");o&&(o.innerHTML='<span class="w-1 h-6 bg-accent-cyan rounded-full"></span> Novo Serviço')}static async editService(e){const i=(await d.getServices()).find(t=>t.id===e);if(!i)return;document.getElementById("editing-service-id").value=i.id,document.getElementById("service-title").value=i.title,document.getElementById("service-desc").value=i.description,document.getElementById("service-full-desc").value=i.full_description||"",document.getElementById("service-img").value=i.image.startsWith("data:")?"":i.image,document.getElementById("service-gallery-urls").value="",this.editingGallery=i.gallery?[...i.gallery]:[],this.renderGalleryPreview();const s=document.querySelector("#tab-services h3");s&&(s.innerHTML='<span class="w-1 h-6 bg-accent-cyan rounded-full"></span> Editar Serviço'),document.getElementById("service-form").scrollIntoView({behavior:"smooth"})}static renderGalleryPreview(){const e=document.getElementById("service-gallery-preview");e&&(e.innerHTML=this.editingGallery.map((a,i)=>`
            <div class="relative w-16 h-16 rounded border border-glass-border overflow-hidden bg-white/5 group">
                <img src="${a}" class="w-full h-full object-cover">
