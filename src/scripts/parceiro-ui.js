...
export class ParceiroUI {
    static async init() {
        ...
        await this.renderPartnerDetails(partnerId);
    }

    static async renderPartnerDetails(id) {
        ...
        container.innerHTML = `
            <div class="flex flex-col gap-12 animate-fade-in-up">
                ...
                <div class="bg-secondary-dark border border-glass-border p-8 rounded-2xl flex flex-col md:flex-row items-center gap-8 hover:border-accent-cyan transition-colors group">
                    <img src="${partner.logo || 'assets/favicon.png'}" alt="${partner.name}" class="h-24 w-auto object-contain bg-white/5 p-4 rounded-xl border border-white/10 group-hover:scale-105 transition-transform">
                    <div class="text-center md:text-left">
                        <span class="text-accent-cyan font-bold uppercase tracking-widest text-xs mb-2 block">Mantenedor Parceiro</span>
                        <h1 class="font-heading font-black text-4xl md:text-5xl text-white tracking-tighter">${partner.name}</h1>
                    </div>
                </div>
                    ...
                    <div class="space-y-8 order-2 lg:order-1">
                        <div class="prose prose-invert max-w-none">
                            ...
                            <p class="text-gray-400 leading-relaxed text-lg">
                                ${partner.description || 'Este parceiro ainda não possui uma descrição detalhada.'}
                            </p>
                        </div>
